...
Après avoir modifié mon CV et ma lettre de motivation, j'ai commencé à recevoir des offres d'emploi, mais elles ne me convenaient pas pour poursuivre le processus de formation. J'ai parcouru <!-- pas sûr qu'en français on puisse "parcourir" un entretien. Il faut changer ce verbe. "eu"? "fait"? ou bien tu veux die "browse"? je ne comprends pas très bien --> plusieurs entretiens et noté les entreprises où je pourrais essayer d'obtenir un emploi après la fin de ma formation. En parallèle, j'ai diffusé activement mon CV aux entreprises de Grenoble et ses environs, y compris les offres envoyées par les responsables de notre master, et je me suis renseignée également sur les postes disponibles parmi les professeurs de mon université. Au total, j'ai postulé à plus de 70 endroits pour toute la recherche. Finalement, j'ai trouvé un stage dans le laboratoire GAEL avec Paolo Crosetto qui a été chargé du cours d'analyse de données en R que j'ai suivi. Ayant déjà 4 ans d'expérience dans le domaine scientifique en Russie et cherchant la possibilité de poursuivre mon activité dans le niveau mondial <!-- plutôt: "à l'international" -->, j'étais convaincue que ce stage s'intègre parfaitement dans mon projet professionnel. 
...
Considérons un exemple pour montrer comment calculer le *r* pour *Investment Game* RET. Soit la montant disponible est de 50, et lors de l'investissement, le montant peut augmenter de 3,5 fois avec une probabilité de 50%, alors:  \((50 - x)^r + 0.5 * (3 * x)^r\). Pour obtenir équation dépendant du choix d'un répondant, il faut maximiser cette fonction et puis la résoudre par rapport à l'appétit pour le risque (*r*). <-- ??? je ne comprends pas cette phrase: --> Le montant de la montant disponible investie est mis dans l'expression résultante pour chaque répondant. <!-- jusqu'ici -->
...
Il a été mentionné ci-dessus que la base de données contient des données sur les répondants, à savoir le sexe, l'âge et le pays de participation à l'expérience. Pour répondre au problème posé, il est nécessaire de mettre en évidence certaines variables qui permettront de décrire les tâches. Et donc, 8 nouvelles variables ont été notées pour chaque RET et implementation du RET : `is_visual`, is_price_list, probabilities_change, stakes et variables dummy pour chaque type de tâche - BRET, BART, EG, IG, où HL est la référence. Ci-dessous il y a un tableau de toutes les variables explicatives pour cette étude. <!-- j'ai pas fait pour toutes les variables mais il est bien de mettre les noms de variables en monospace, donc en utilisant `VARNAME` en Rmd -->
...
Si nous voulons minimiser la différence entre les séries, on a besoin que les points de neutralité du risque de deux distributions coïncident. Pour CRRA = 1, la valeur d'une nouvelle échelle de y (y_r_DospertScale) est égale à 4.83352. On sait aussi que le répondant neutre au risque obtient 4 sur l'échelle DOSPERT réelle. Soustrayons la valeur 0.83352 des valeurs de la série obtenu. Puisque nous avons soustrait la valeur, la queue droite a été mise à zéro dessus <!-- ???? "a été mise à zéro dessus" ???? -->. On peut tronquer la distribution sur la queue droite pour ne faire correspondre que les réponses réelles. Après une telle transformation, `r nrow(df_test_dosp)` observations sont restées dans l'échantillon. Maintenant on peut comparer ces deux distributions (Figure 3).
...
Au cours de cette étude, on a tenté de construire <!-- je ne comprends aps cette phrase. Elle paraît dire "look, we tried this". Mais arrivés à ce point, il faut dire "donc, pour répondre à nos questions, et avec tout ce qu'on a construit, on peut finalement faire une estiamtiona vec ce modèele là". Etre plus sûr de soi et faire suivre logiquement un pas méthodologique à l'autre, convaincre le lecteur que c'est la bonne méthode; c'est ça que tes mots doivent faire. --> un modèle de régression linéaire pour chaque type de questionnaire sur les certaines variables explicatives choisies pour l'analyse :
...
Pour la comparabilité des résultats, un échantillon a été créé à partir duquel les valeurs aberrantes de SOEP et de DOSPERT ont été exclues. Il contenait `r nrow(df_test)` observations. Pour vérifier la robustesse des résultats, les modèles avec les échantillons complets pour chaque type de questionnaire sont également donnés. <!-- où? J'imagine sur les colonnes 2 et 4 de ta régression... si c'est ça: décrit mieux tes colonnes dans le tableau aussi; je sais que tu l'écris plus bas, mais tables and plots should be able to stand on their own (une des règles qui sont appliqués par les revues scientifiques -->
